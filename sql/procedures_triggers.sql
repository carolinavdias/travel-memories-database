-- ============================================
-- Mapa de Memórias — Procedures, Functions & Triggers
...
CREATE OR REPLACE FUNCTION fn_media_avaliacao_experiencia(
...
CREATE OR REPLACE PROCEDURE sp_criar_experiencia(
    IN p_id_autor INT,
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE u.id_utilizador = p_id_autor AND u.estado_conta = 'ativo'
    ) THEN
        SIGNAL SQLSTATE '45000'
    ...
    INSERT INTO EXPERIENCIA(
        ...
        data_inicio, data_fim, tipo_experiencia, data_criacao, estado
    ) VALUES (
        p_id_autor, v_id_destino, p_titulo, p_descricao,
        p_data_inicio, p_data_fim, p_tipo_experiencia, NOW(), 'ativa'
    );
END$$

-- Registo de avaliação
CREATE OR REPLACE PROCEDURE sp_registar_avaliacao(
...
BEGIN
    ...
    VALUES (p_id_utilizador, p_id_experiencia, p_classificacao, NOW());
END$$

-- Adicionar comentário
CREATE OR REPLACE PROCEDURE sp_adicionar_comentario(
    IN p_id_utilizador INT,
    IN p_id_experiencia INT,
    IN p_texto TEXT
)
BEGIN
    ...
    VALUES (p_id_utilizador, p_id_experiencia, p_texto, NOW());
END$$

-- Soft delete da experiência
CREATE OR REPLACE PROCEDURE sp_remover_experiencia(
    IN p_id_experiencia INT,
    IN p_id_autor INT
)
BEGIN
...
CREATE OR REPLACE TRIGGER trg_experiencia_validar_bi
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM EXPERIENCIA e
        WHERE e.id_experiencia = NEW.id_experiencia AND e.estado = 'ativa'
    ) THEN
...
CREATE OR REPLACE TRIGGER trg_comentario_validar_bi
